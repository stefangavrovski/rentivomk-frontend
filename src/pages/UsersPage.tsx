...
export default function UsersPage() {
  ...
  return (
    <div>
      <PageHeader title="Users" subtitle="Manage all registered users and their roles" />
        ...
        <div className="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1">
          {ROLE_FILTERS.map(r => (
            <button key={r} onClick={() => setFilter(r)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filter === r ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}>
              {r}
            </button>
          ))}
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          {[...Array(5)].map((_, i) => (
            <div key={i} className={`flex items-center gap-4 px-5 py-4 animate-pulse ${i !== 0 ? 'border-t border-slate-800' : ''}`}>
              ...
              <div className="flex-1 space-y-2"><div className="h-3.5 bg-slate-800 rounded w-1/4" /><div className="h-3 bg-slate-800 rounded w-1/3" /></div>
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-slate-600 text-sm mt-1">{filter !== 'All' || search ? 'Try adjusting your filters.' : 'No users are registered yet.'}</p>
        </div>
      )}
      ...
      {!loading && filtered.length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          {filtered.map((u, i) => (
            <div key={u.id} className={`flex items-center gap-4 px-5 py-4 hover:bg-slate-800/40 transition-colors ${i !== 0 ? 'border-t border-slate-800' : ''}`}>
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border shrink-0 ${roleBadge[u.role]}`}>{u.role}</span>
              <p className="text-slate-600 text-xs shrink-0 hidden sm:block">Joined {formatDate(u.createdAt)}</p>
              <div className="flex items-center gap-1.5 shrink-0">
                <button onClick={() => setEditTarget(u)} className="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-400 hover:text-amber-400 hover:bg-amber-500/10 border border-slate-700 hover:border-amber-500/30 transition-all">
                  Edit
                </button>
                <button onClick={() => setDeleteTarget(u)} disabled={u.email === currentUser?.email}
      ...
      {editTarget && <UserFormModal user={editTarget} onSubmit={handleUpdate} onClose={() => setEditTarget(null)} />}
      {deleteTarget && (
        <ConfirmDialog
          title="Delete User"
