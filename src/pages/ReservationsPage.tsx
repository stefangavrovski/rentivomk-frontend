...
const ACTION_CONFIG = {
  approve:  { label: 'Approve',  confirmLabel: 'Approve',  title: 'Approve Reservation',  message: (r: ReservationDto) => `Approve the reservation for ${r.vehicleName} by ${r.customerName}?`,         btnClass: 'bg-emerald-500 hover:bg-emerald-400 text-white' },
  reject:   { label: 'Reject',   confirmLabel: 'Reject',   title: 'Reject Reservation',   message: (r: ReservationDto) => `Reject the reservation for ${r.vehicleName} by ${r.customerName}?`,          btnClass: 'bg-red-500 hover:bg-red-400 text-white' },
  complete: { label: 'Complete', confirmLabel: 'Complete', title: 'Complete Reservation',  message: (r: ReservationDto) => `Mark the reservation for ${r.vehicleName} as completed?`,                   btnClass: 'bg-blue-500 hover:bg-blue-400 text-white' },
  cancel:   { label: 'Cancel',   confirmLabel: 'Cancel',   title: 'Cancel Reservation',   message: (r: ReservationDto) => `Cancel the reservation for ${r.vehicleName} by ${r.customerName}?`,         btnClass: 'bg-red-500 hover:bg-red-400 text-white' },
};
...
export default function ReservationsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Reservations"
        subtitle="Manage all customer reservations"
      />
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-slate-600 text-sm mt-1">
            {filter !== 'All' || search ? 'Try adjusting your filters.' : 'No reservations have been made yet.'}
          </p>
        </div>
      )}
                    ...
                    {actions.length > 0 && (
                      <div className="flex items-center gap-1.5 flex-wrap justify-end">
                        {actions.map(action => (
                          <button
                            key={action}
                            onClick={() => setPendingAction({ type: action, reservation: r })}
                            className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${
                              action === 'approve'
                                ? 'text-emerald-400 hover:bg-emerald-500/10 border-slate-700 hover:border-emerald-500/30'
                                : action === 'complete'
                                ? 'text-blue-400 hover:bg-blue-500/10 border-slate-700 hover:border-blue-500/30'
