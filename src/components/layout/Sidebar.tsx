...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-1.5-4.5a1 1 0 00-.95-.5H7.45a1 1 0 00-.95.5L5 9M3 12v6a1 1 0 001 1h1a1 1 0 001-1v-1h12v1a1 1 0 001 1h1a1 1 0 001-1v-6M3 12h18M7.5 15.5h.01M16.5 15.5h.01" /></svg>,
  },
  {
    label: 'My Reservations', to: '/reservations/my',
    roles: ['Customer'],
    ...
    <aside className="h-screen w-64 bg-slate-900 border-r border-slate-800 flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-amber-500 shadow-md shadow-amber-500/30 shrink-0">
          ...
          <div>
            <p className="text-white font-bold text-sm leading-tight">RentivoMK</p>
            <p className="text-slate-500 text-xs">Vehicle Rental</p>
          </div>
        </div>
        {onClose && (
          <button onClick={onClose} className="lg:hidden p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors" aria-label="Close menu">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                isActive ? 'bg-amber-500 text-slate-950 shadow-md shadow-amber-500/20' : 'text-slate-400 hover:text-white hover:bg-slate-800'
              }`
            }
          >
            {item.icon}
